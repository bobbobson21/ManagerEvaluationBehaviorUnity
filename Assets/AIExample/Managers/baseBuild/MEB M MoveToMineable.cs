...
#if UNITY_EDITOR
[InitializeOnLoad]
public class UserManger_BBMoveToMineable_UI : MEB_UI_BehaviourEditor_ManagerData
{
    static UserManger_BBMoveToMineable_UI()
    {
        MEB_UI_BehaviourEditor.AddNormalManager(new UserManger_BBMoveToMineable_UI());
    }

    public UserManger_BBMoveToMineable_UI()
    {
        m_name = "UserManger_BBMoveToMineable";
    }

    public override MEB_BaseBehaviourData_ItemSettings CreateInstance()
    {
        MEB_BaseBehaviourData_ItemSettings data = new MEB_BaseBehaviourData_ItemSettings();
        data.m_class = "UserManger_BBMoveToMineable";
        data.m_displayName = m_name;
        data.m_displayDiscription = "Move to the nearest minable object. \n\nvaild blackboard data: \nstoreTargetLocationIn: (vector3BlackboardKeyAsString)\ngetMineObjectFrom: (gameObjectBlackboardKeyAsString)";
...
public class UserManger_BBMoveToMineable : MEB_BaseManager, MEB_I_IntScoop
{
    private string m_storeTargetLocationInKey = "";
    private string m_getMineObjectFromKey = "";
    ...
    public override void SetBlackboardKeys(List<string> idenifyers, List<string> keys)
    {
        for (int i = 0; i < idenifyers.Count; i++)
        {
            ...
            if (idenifyers[i] == "getMineObjectFrom")
            {
                m_getMineObjectFromKey = keys[i];
            }
        }
    }
    ...
    {
        GameObject obj = (GameObject)m_director.m_blackboard.GetObject(m_getMineObjectFromKey);
    ...
    {
        if (m_director.m_blackboard.GetObject(m_getMineObjectFromKey) == null)
        {
            return 10;
        }
