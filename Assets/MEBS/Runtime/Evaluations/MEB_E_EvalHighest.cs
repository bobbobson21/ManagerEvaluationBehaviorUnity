...
namespace MEBS.Editor
{
    ...
    public class MEB_E_EvalHighest_UI : MEB_UI_BehaviourEditor_ManagerData
    {
        ...
        public override MEB_BaseBehaviourData_ItemSettings CreateInstance()
        {
            ...
            data.m_displayDiscription = "Gets all the managers in the 'managers to evalurate' section to return an int thougth MEB_I_IntScoop.GetIntEvalValue(float delta). The one that returns the higest value, that also isn't already blocked, moves on the rest are blocked from execution.";
...
{
    public class MEB_E_EvalHighest : MEB_BaseManager, MEB_I_EvalScoop
    {
        ...
        public override void EvaluationStart(int index, float delta)
        {
            ...
            for (int i = 0; i < arrayLength; i++) //find highest value
            {
                try
                {
                    ...
                    if (manager.IsAllowedToExecute() == true && testValue > highestValueSoFar)
                    {
